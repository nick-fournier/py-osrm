...
def generate_report(
    ...
    if batch_log:
        ...
        descriptions.append(
            ...
            "active links. Lower = more uniform utilization (better route "
            "distribution). High values mean congestion is concentrated on "
            "a few links while others are underused.</li>"
            "<li><b>Mean V/C</b> — Average volume-to-capacity ratio across "
            "active links. Drives dynamic batch sizing: "
            "<code>batch_scale = k / (mean_vc + k)</code>.</li>"
            "<li><b>Flow Stability (Flow Δ‖·‖)</b> — Normalized flow change "
            "between consecutive period-finals: "
            "<code>‖flow<sub>N</sub> − flow<sub>N−1</sub>‖ / "
            "‖flow<sub>N</sub>‖</code>. "

...
| Density model | **Mesoscopic with spatial smoothing** (§3.8) | Neighbor-based smoothing avoids short-link volatility; handles spillover |
| CSV I/O (OSRM) | **tmpfs (`/dev/shm/`)** for prototype ([py-osrm Assignment Module](pyosrm_assignment_module.md) §1.6) | Zero disk I/O; no OSRM changes needed; future: in-memory bypass |
| Engine refresh (OSRM) | **Destroy/recreate** (prototype), **shared-memory hot-swap** (production, [py-osrm Assignment Module](pyosrm_assignment_module.md) §1.5) | Hot-swap is zero-downtime but requires `osrm-datastore` integration |
...
> **Note**: The OSRM traffic update surface (how `customize()` updates edge
> weights via CSV) is documented in
> [py-osrm Assignment Module](pyosrm_assignment_module.md) §1.

> **Note**: The complete OSRM multi-period metric patch specification is in
> [OSRM Multi-Period Patch](osrm_multi_period_patch.md).

> **Note**: Path-to-link accounting (mapping OSRM route annotations to
> assignable edges) is documented in
> [py-osrm Assignment Module](pyosrm_assignment_module.md) §2.

> **Note**: Network state data structures (link state variables, defaults,
> `NetworkState` class) are documented in
> [py-osrm Assignment Module](pyosrm_assignment_module.md) §3.

---

## 2  Flow accumulation semantics: why occupancy is already correct
...
MFD-based smoothing (see §3.8).

### 3.2  Functional form (from Fournier et al.)
...
### 3.4  Default parameters (from Fournier et al.)
...
### 3.5  From density to OSRM weights
...
- **If q_e > q_c**: the link is oversaturated. See §3.7.
...
## 4  Temporal model: discrete time slices

### 4.1  Slice lifecycle (overview)

The assignment processes all demand across all time bins simultaneously, then
distributes flow to bins via travel-time offsets (see §4.3 for details).
...
For each outer iteration:
  ...
  4. CSV        Write updated speeds to /dev/shm/ ([py-osrm Assignment Module](pyosrm_assignment_module.md) §1.6)
  5. CUSTOMIZE  Run osrm.customize() with segment-speed-file
  6. RELOAD     Refresh engine (Strategy A or B, [py-osrm Assignment Module](pyosrm_assignment_module.md) §1.5)
...
- **Frozen-cost accuracy**: Most urban trips (< 45 min) complete within one
  ...
  cross-regional trips span multiple periods (see [OSRM Multi-Period Patch](osrm_multi_period_patch.md) §5 for implications).
...
- **Memory**: Per-period overhead scales linearly with N. At 24 hourly
  periods with sparse cell-delta storage ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §7), California fits in ~10–12
...
period width is user-configurable via `PeriodConfig` ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §6).

Sensitivity analysis on period width is a key validation task.

### 4.3  Multi-bin trips: fractional link loading
...
The slice lifecycle (§4.1) changes: instead of processing one bin at a time
...
4. For each bin t = 0, 1, ..., T-1:
   ...
   b. DENSITY    Convert flow to density (§3.6)
   c. SMOOTH     Neighbor-based smoothing (§3.8)
...
### 4.4  Inner convergence loop (optional)
...
assigner = osrm.TrafficAssignment(
    ...
    density_smoothing="zone",     # "zone", "neighbor", or "none" (§3.8)
    smoothing_lambda=0.6,         # blending parameter
    csv_backend="tmpfs",          # "tmpfs" (/dev/shm/), "disk", or "memory"
    engine_refresh="destroy",     # "destroy" (Strategy A) or "hotswap" (Strategy B, [py-osrm Assignment Module](pyosrm_assignment_module.md) §1.5)
)
...
### 5.3  Matrix-free trip adapter (second)
...
With the `departure_time` parameter in the OSRM patch ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §4), the
...
> **Note**: Concrete implementation gaps and the new assignment module
> structure are documented in
> [py-osrm Assignment Module](pyosrm_assignment_module.md) §4.
...
### 6.2  Unit tests (pure Python, no OSRM dependency)
...
### 6.3  Structural validation: Braess network
...
### 6.4  Benchmark validation: Sioux Falls
...
### 6.5  Benchmark validation: Nguyen-Dupuis (optional)
...
Same validation properties as §6.4, items 1–4.

### 6.6  Empirical validation against observed traffic data
...
### 6.7  Regional runtime benchmarking
...
### 6.8  Expected performance characteristics
...
cell-delta optimization ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §7) reduces output size but not customize
compute time itself.
...
Unit tests (§6.2) and the Monaco smoke benchmark (§6.6 Tier 1) run in CI
on every PR. Structural validation (Braess, §6.3) also runs in CI — the
synthetic OSM is tiny and processes in seconds.

Sioux Falls validation (§6.4) runs in CI but with relaxed iteration limits
...
- `BiParabolicVDF`: vectorized NumPy implementation (§3.2–3.7)
- `DensitySmoothing`: neighbor-based smoothing (§3.8)
- `SegmentSpeedWriter`: generate CSV to tmpfs `/dev/shm/` ([py-osrm Assignment Module](pyosrm_assignment_module.md) §1.6)
- `FractionalLoader`: travel-time offset bin distribution (§4.3)
- Unit tests for all of the above (§6.2)
- TNTP-to-OSM bridge utility for test network synthesis (§6.4)
...
- Braess paradox validation (§6.3) — synthesize OSM, verify paradox manifests
- Monaco smoke benchmark (§6.6 Tier 1)
- CI integration for unit tests + Braess + Monaco

### Phase 4: Sioux Falls validation and matrix-free adapter

**Deliverable**: Sioux Falls benchmark validation (§6.4). Trip-stream demand
...
**Deliverable**: The OSRM core patch ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §4) enabling multi-period metric
storage, `departure_period` and `departure_time` query parameters (both
C++ library and HTTP API). Multi-period DTA on OSRM.

**Scope**:
- Implement the ~350-line OSRM core patch ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §4):
...
- Sparse cell-delta storage ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §7) as optimization pass
- Backward compatibility: existing OSRM data (no period entries) loads as
  single period (period_index=0)
- `PeriodConfig` Python class for user-defined period mappings ([OSRM Multi-Period Patch](osrm_multi_period_patch.md) §6)
- Multi-period assignment loop ([py-osrm Assignment Module](pyosrm_assignment_module.md) §5)
...
**Scope**:
- Metro-scale benchmark (§6.7 Tier 2) — Geofabrik extract, synthetic demand
- Benchmark customize latency at scale (tmpfs CSV vs. disk, [py-osrm Assignment Module](pyosrm_assignment_module.md) §1.6)
- Parallelize period customization across cores
- C++ flow-accumulation extension if Python is bottleneck
- Evaluate shared-memory hot-swap ([py-osrm Assignment Module](pyosrm_assignment_module.md) §1.5, Strategy B) — wrap
...
- Region-scale stretch test (§6.7 Tier 3) — California or similar
...
| **Engine re-instantiation has hidden side effects** (TBB thread pool, memory leaks) | Medium | Medium | py-osrm already has a TBB cleanup handler; test repeated create/destroy cycles; migrate to shared-memory hot-swap ([py-osrm Assignment Module](pyosrm_assignment_module.md) §1.5, Strategy B) if problematic |
...
1. **Slice width calibration**: 1-hour periods are recommended as the default
   (§4.2). Should we run a sensitivity analysis across 15/30/60 min as part
...
Mesoscopic density smoothing (see §3.8):
